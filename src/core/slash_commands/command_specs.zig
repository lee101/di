...
pub fn renderTopLevelHelpWithStyle(alloc: Allocator, registry: TopLevelRegistry, columns: usize, version: []const u8, style: HelpStyle) ![]u8 {
    ...
    try writeStyled(&out.writer, style, .brand, "di");
    ...
    try writeWrappedStyledLine(&out.writer, "  ", "  ", "di [flags]", width, style, .syntax);
    try writeWrappedStyledLine(&out.writer, "  ", "  ", "di <command> [...flags] [...args]", width, style, .syntax);
...
test "rendered top-level help is a complete CLI navigation page" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, text, "di v9.8.7\nFast, native coding agent for the terminal."));
    try std.testing.expectEqual(@as(usize, 1), std.mem.count(u8, text, "di v9.8.7"));
    try std.testing.expect(std.mem.find(u8, text, "fx starts an interactive session by default.") != null);
    try std.testing.expect(std.mem.find(u8, text, "di <command> [...flags] [...args]") != null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "di ask \"Explain the changes in this repository\"") != null);
    try std.testing.expect(std.mem.find(u8, text, "di session resume last") != null);
    try std.testing.expect(std.mem.find(u8, text, "session resume [last|id]") != null);
    try std.testing.expect(std.mem.find(u8, text, "di status --json") != null);
...
test "terminal top-level help adds styling without changing visible content" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, terminal, "\x1b[1mdi\x1b[0m"));
...
test "top-level help hides developer recording surfaces" {
    ...
    try std.testing.expect(std.mem.find(u8, replay, "di replay") != null);
}
...
test "per-command help renders header usage options and details" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "di permissions\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  di permissions [--json]") != null);
...
test "per-command help preserves long resume usage without debug recording" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  di session resume [last|<id>] | session resume --id <id> | --resume [last|<id>] | resume [last|<id>] | resume --id <id> | --resume-last | --continue | -c | -r | --resume-<id>") != null);
    try std.testing.expect(std.mem.find(u8, text, "Options:") != null);
    try std.testing.expect(std.mem.find(u8, text, "--record") == null);
}
...
test "slash completion descriptions follow completion matches" {
    ...
    try std.testing.expectEqualStrings("open the di feedback form", nthSlashCompletionDescription(testSlashRegistry(), "/fee", 0).?);
