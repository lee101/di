...
pub const ModelOrigin = enum {
    ...
    pub fn provider(self: ModelOrigin) model_provider.ProviderId {
        return switch (self) {
            ...
            .fx_login, .ai_gateway_api_key, .vercel_oidc_token, .stored_key, .host_managed => .gateway,
            .configured => .{ .configured = .{ .len = 0 } },
        };
    }
    ...
    pub fn requiresSwitch(
        ...
        if (!active_provider.eql(self.provider())) return true;
...
test "model menu provider navigation skips absent and redundant filters" {
    ...
    try hydrateMenuSnapshot(alloc, &mixed, &mixed_entries, &.{}, null);
    mixed.active = true;
    ...
    try hydrateMenuSnapshot(alloc, &codex, &codex_entries, &.{}, null);
    codex.active = true;
