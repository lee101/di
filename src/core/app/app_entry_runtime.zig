...
fn runInteractiveWithDeps(comptime App: type, comptime cooperative: bool, alloc: Allocator, launch: *cli_surface.InteractiveLaunch, auth_mode: credentials.AuthMode, deps: RunDeps) !RunOutcome {
    ...
    var app = App.init(alloc, launch, auth_mode) catch |err| {
        switch (err) {
            ...
            error.RecordingStartFailed => {
                writeStderr(deps, "di: unable to start terminal recording.\n");
                return .{ .exit = 1 };
            },
            ...
            error.SessionNotFound => {
                writeStderr(deps, "di: saved session not found.\n");
                return .{ .exit = 1 };
            },
            error.SessionBusy => {
            ...
            error.SessionLockUnsupported => {
                writeStderr(deps, "di: the filesystem cannot provide the required session lock\n");
                return .{ .exit = 1 };
            },
            ...
            error.InvalidSessionFormat => {
                writeStderr(deps, "di: saved session is unreadable. Run `di doctor`; if it is recoverable, use `di session recover <id>`.\n");
                return .{ .exit = 1 };
            },
    ...
    if (relaunch_request) |request| {
        if (handoff_value) |value| {
            ...
            writeStderr(
                deps,
                "di: upgrade installed, but no validated resume handoff was available. Your conversation remains on disk; run `di doctor`.\n",
            );
        }
        return .{ .exit = 1 };
    }
...
fn writeUpgradeRelaunchFailure(
    ...
    const message = std.fmt.bufPrint(
        &buffer,
        "di: upgrade installed, but relaunch failed: {s}\nContinue session with: di --resume {s}\n",
        .{ @errorName(err), session_id },
    ) catch "di: upgrade installed, but relaunch failed; run `di doctor`.\n";
...
fn formatUnexpectedError(buffer: []u8, err: anyerror) ![]const u8 {
    return std.fmt.bufPrint(buffer, "di: {s}\n", .{@errorName(err)});
}
...
test "app entry reports unexpected init errors once and preserves identity" {
    ...
    try std.testing.expectEqualStrings("di: TestInitFailed\n", capture.stderr.written());
...
test "app entry releases terminal before reporting worker start errors" {
    ...
    try std.testing.expectEqualStrings("di: TestWorkerStartFailed\n", capture.stderr.written());
    try std.testing.expectEqual(@as(usize, 1), capture.stderr_calls);
...
test "app entry reports run errors before deinit and outer cleanup" {
    ...
    try std.testing.expectEqualStrings("di: TestRunFailed\n", capture.stderr.written());
    try std.testing.expectEqual(@as(usize, 1), capture.stderr_calls);
    try std.testing.expectEqual(@as(usize, 0), capture.stdout_calls);
...
test "app entry maps unavailable session state to one expected startup failure" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .message = "di: the filesystem cannot provide the required session lock\n",
        },
        .{
