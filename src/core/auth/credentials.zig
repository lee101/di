...
pub const missing_credential_message = "di needs access to Vercel AI Gateway. Run di login to sign in, di setup to use an API key, or set AI_GATEWAY_API_KEY.";
pub const missing_interactive_credential_message = "di needs access to Vercel AI Gateway. Run /login to sign in, /provider to use an API key, or set AI_GATEWAY_API_KEY.";
pub const missing_chatgpt_credential_message = "di needs a Codex subscription login for this model. Run di login codex.";
...
pub const missing_grok_credential_message = "di needs a Grok subscription login for this model. Run di login grok.";
...
pub const unreadable_store_message = "di could not read the stored API key from " ++ stored_key_backend_label ++ ". A key may be saved but unreadable. Set FX_TRACE_LOG for the failing step, or set AI_GATEWAY_API_KEY.";
pub const host_managed_auth_message = "Authentication is managed by the host.";

test "public credential guidance spells di lowercase" {
    try std.testing.expect(std.mem.startsWith(u8, missing_credential_message, "di needs"));
    try std.testing.expect(std.mem.startsWith(u8, missing_interactive_credential_message, "di needs"));
    try std.testing.expect(std.mem.startsWith(u8, unreadable_store_message, "di could"));
}
